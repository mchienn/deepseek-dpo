"""
eval_deepseek_v2.py — Evaluate DeepSeek-R1-Distill-Qwen-1.5B LoRA adapter (Version 2)
──────────────────────────────────────────────────────────────────────────
Pipeline:
  1. Load base model + LoRA adapter v2 (merged)
  2. Inference on test.xlsx  (Input → Reasoning → JSON output)
  3. Score JSON output vs ground truth using ScoreEval logic
  4. Save results to eval_deepseek_result_v2.xlsx
──────────────────────────────────────────────────────────────────────────
"""
...
ADAPTER_DIR    = "./dpo_adapter_v2"
TEST_FILE      = "test_cleaned.xlsx"
OUTPUT_FILE    = "eval_deepseek_result_v2.xlsx"
THRESHOLD      = 0.7
WEIGHT_VALUE   = 1
MAX_NEW_TOKENS = 1024
HF_BATCH       = 256
...
def run_hf_inference(subtasks: list) -> list:
    CKPT = Path("eval_checkpoint_v2.jsonl")
...
def main():
    # Auto-tee stdout/stderr to a log file
    log_path = f"eval_v2_{time.strftime('%Y%m%d_%H%M%S')}.log"
    ...
    print("=" * 60)
    print("  DeepSeek-R1-Distill Evaluation (v2)")
    print("=" * 60)
