...
SEED="${4:-42}"`nDPO_BATCH_SIZE="${DPO_BATCH_SIZE:-4}"`nDPO_GRADIENT_ACCUMULATION="${DPO_GRADIENT_ACCUMULATION:-4}"
...
python3 train_dpo_v5.py --run-id "$RUN_ID" --pairs "runs/${RUN_ID}/pairs.jsonl" --sft-adapter "$SFT_ADAPTER" --beta "$BETA" --learning-rate "$LEARNING_RATE" --seed "$SEED" --batch-size "$DPO_BATCH_SIZE" --gradient-accumulation "$DPO_GRADIENT_ACCUMULATION"
